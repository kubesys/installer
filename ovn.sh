...
IFACE="eth0"                               # The nic to support container network can be a nic name or a group of regex separated by comma, if empty will use the nic that the default route use

REGISTRY="registry.cn-beijing.aliyuncs.com/dosproj"
VERSION="v1.6.0"
...
if [[ $ENABLE_SSL = "true" ]];then
  ...
  if [[ $exist == "" ]];then
    docker run --rm -v "$PWD":/etc/ovn $REGISTRY/alauda-kubeovn:$VERSION bash generate-ssl.sh
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: ovn-central
          image: "$REGISTRY/alauda-kubeovn:$VERSION"
